...
public class Controller {
    ...
    public void changePassword(){
        ...
        int count = 0;
        for (int i = 0; i < users.size(); i++){
            if(password.equals(users.get(i).getPassword()) && username.equals(users.get(i).getUser_name())){
                count++;
                boolean check = false;
    ...
    public void changeUsername(){
        ...
        while (!check){
            try {
                ...
                for (int i = 0; i < users.size(); i++){
                    if(users.equals(users.get(i).getUser_name())){
                        if(!newUsername.equals(users.get(i).getUser_name())){
                            users.get(i).setUser_name(newUsername);
    ...
    public void changeEmail(){
        ...
        while (!check){
            try {
                ...
                System.out.println(e.getMessage()+ ", vui lòng đăng nhập lại email");
            }
        }
        loginSuccess();
    }
    ...
    public void forgotPassword(){
        ...
        if(count == 0){
            System.out.println("Email không tồn tại, hãy đăng nhập lại");
            forgotPassword();
        }
    }
    public void createNewAccount(){
        ...
        while (!check){
            try{
                ...
                newPassword = Validate.validatePassword(scanner.nextLine());
                for (int i = 0; i < users.size(); i++){
                    if(newUsername.equals(users.get(i).getUser_name())) throw new RuntimeException("Username đã tồn tại");
